...
class DTree:
    ...
    def _no_repeats_helper(self, seen):
        ...
        # Breaking long conditions into multiple lines
        lessequal_valid = True
        if self.lessequal is not None:
            lessequal_valid = self.lessequal._no_repeats_helper(seen.copy())

        greater_valid = True
        if self.greater is not None:
            greater_valid = self.greater._no_repeats_helper(seen.copy())
